...
static struct class* gib_class; //class for gps interface block.
static dev_t dev;
...
static int gib_dev_init(void)
{
	...
	gib_class = class_create(THIS_MODULE,"gps");
	if(IS_ERR(gib_class)){
		printk("\n cannot create gps class!!\n");
		gib_class = NULL;
	}
	
	dev = MKDEV(GIB_MAJOR,0); 
	device_create(gib_class,NULL,dev,"%s","dev_gib");
...
static void gib_dev_exit(void)
{
	CRDEBUG;
	#if 0
	device_destroy(gib_class,dev);
	class_destroy(gib_class);
	#endif
