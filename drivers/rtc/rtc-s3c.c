...
static int s3c_rtc_settime(struct device *dev, struct rtc_time *tm)
{
	...
	pr_debug("s3c_rtc_settime time [%d.%d.%d]:[%d.%d.%d]\n",
...
static int s3c_rtc_setalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	pr_debug("s3c_rtc_setalarm: enabled[0x%x] time [%d.%d.%d]:[%d.%d.%d]\n",
		 alrm->enabled,
		 tm->tm_year, tm->tm_mon, tm->tm_mday,
		 tm->tm_hour, tm->tm_min, tm->tm_sec);
...
static int __devinit s3c_rtc_probe(struct platform_device *pdev)
{
	...
	if (!(readw(s3c_rtc_base + S3C2410_RTCCON) & S3C2410_RTCCON_RTCEN)) {
		/* RTC disabled => value undefined, set it to 01.01.2000 00:00:00 */
