...
struct sensor_regulator lis3dh_acc_vreg[] = {
	{NULL, "vmmc1", 1700000, 3600000},
	{NULL, "vaux4", 1700000, 3600000},
};
					...
					int *xyz)
{
	...
	input_report_abs(acc->input_dev, ABS_MISC, jiffies);
...
#ifdef USE_HRTIMER
		hrtimer_start(&acc->worker.timer,
			ktime_set(0, acc->pdata->poll_interval * 1000000),
			HRTIMER_MODE_REL);
#else
...
#ifdef USE_HRTIMER
		hrtimer_cancel(&acc->worker.timer);
		acc->worker.wake_up = 0;
#else
...
static ssize_t attr_get_avg_samples(struct device *dev,
				     ...
				     char *buf)
{
	...
	struct lis3dh_acc_data *acc = dev_get_drvdata(dev);
	mutex_lock(&acc->lock);
	val = acc->pdata->avg_samples;
	mutex_unlock(&acc->lock);
	return snprintf(buf, 8, "%d\n", val);
}

static ssize_t attr_set_avg_samples(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t size)
{
	...
	avg_samples = simple_strtoul(buf, NULL, 10);
	if (!avg_samples)
		return -EINVAL;
	mutex_lock(&acc->lock);
	acc->pdata->avg_samples = avg_samples;
	mutex_unlock(&acc->lock);
	return size;
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(avg_samples, 0666, attr_get_avg_samples,
			attr_set_avg_samples),
...
#ifdef USE_HRTIMER
static int lis3dh_worker(void *data)
{
	struct lis3dh_acc_data *acc = (struct lis3dh_acc_data *)data;
	int xyz[3] = { 0 };
	int err;

	do {
		wait_event_interruptible(acc->worker.wq,
					acc->worker.wake_up ||
					kthread_should_stop());
		if (acc->worker.wake_up) {
			acc->worker.wake_up = 0;
			mutex_lock(&acc->lock);
			...
			if (err < 0) {
				...
				if (acc->sample_count == acc->pdata->avg_samples) {
					xyz[0] = acc->xyz_sum[0] / acc->sample_count;
					xyz[1] = acc->xyz_sum[1] / acc->sample_count;
					xyz[2] = acc->xyz_sum[2] / acc->sample_count;
...
static enum hrtimer_restart lis3dh_acc_input_timer_func(struct hrtimer *timer)
{
	struct lis3dh_acc_data *acc;
	unsigned long missed;

	acc = container_of(timer, struct lis3dh_acc_data, worker.timer);

	acc->worker.wake_up = 1;
	wake_up(&acc->worker.wq);

	missed = hrtimer_forward_now(&acc->worker.timer,
			ktime_set(0, acc->pdata->poll_interval * 1000000));
	if (missed > 1)
		dev_err(&acc->client->dev, "Missed ticks %ld\n", missed - 1);
...
#ifdef CONFIG_PM
#ifdef CONFIG_EARLYSUSPEND
static void lis3dh_acc_early_resume(struct early_suspend *handler)
{
	struct lis3dh_acc_data *acc;
...
static int lis3dh_acc_validate_pdata(struct lis3dh_acc_data *acc)
{
	...
	/* average samples should greater than 0 */
	if (acc->pdata->avg_samples < 1) {
		dev_err(&acc->client->dev, "average samples should >= 0\n");
		return -EINVAL;
	}
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
...
#ifdef USE_HRTIMER
	acc->worker.wake_up = 0;
	init_waitqueue_head(&acc->worker.wq);

	acc->worker.thread = kthread_run(lis3dh_worker, acc, "lis3dh");
	if ( IS_ERR(acc->worker.thread)) {
		dev_err(&acc->client->dev, "Failed to create worker thread\n");
	}
	hrtimer_init(&acc->worker.timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	acc->worker.timer.function = lis3dh_acc_input_timer_func;
#else
	...
	/*	Use ABS_MISC to indicate sensor is alive */
	set_bit(ABS_MISC, acc->input_dev->absbit);
	/*	next is used for interruptA sources data if the case */
	set_bit(ABS_BRAKE, acc->input_dev->absbit);
	...
	input_set_abs_params(acc->input_dev, ABS_MISC, 0, ULONG_MAX, 0, 0);
	/*	next is used for interruptA sources data if the case */
	input_set_abs_params(acc->input_dev, ABS_BRAKE, INT_MIN, INT_MAX, 0, 0);
	/*	next is used for interruptB sources data if the case */
...
static void lis3dh_acc_input_cleanup(struct lis3dh_acc_data *acc)
{
	...
	kthread_stop(acc->worker.thread);
#endif
}
