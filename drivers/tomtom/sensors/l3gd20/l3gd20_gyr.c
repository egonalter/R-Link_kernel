					...
					struct l3gd20_gyr_triple *data)
{
	...
	input_report_abs(stat->input_dev, ABS_MISC, jiffies);
...
#ifdef DEBUG
			input_report_abs(stat->input_dev, ABS_TOOL_WIDTH, 1);
			input_sync(stat->input_dev);
#endif
...
#ifdef DEBUG
			input_report_abs(stat->input_dev, ABS_TOOL_WIDTH, 0);
			input_sync(stat->input_dev);
#endif
		}
...
#ifdef DEBUG
	input_report_abs(stat->input_dev, ABS_TOOL_WIDTH, 3);
	input_sync(stat->input_dev);
#endif
...
#ifdef DEBUG
	input_report_abs(stat->input_dev, ABS_TOOL_WIDTH, 2);
	input_sync(stat->input_dev);
#endif
...
static int l3gd20_gyr_input_init(struct l3gd20_gyr_status *stat)
{
	...
	set_bit(ABS_MISC, stat->input_dev->evbit);

#ifdef DEBUG
	set_bit(EV_KEY, stat->input_dev->keybit);
	set_bit(KEY_LEFT, stat->input_dev->keybit);
	input_set_abs_params(stat->input_dev, ABS_TOOL_WIDTH, 0, 1, 0, 0);
#endif
	...
	input_set_abs_params(stat->input_dev, ABS_MISC, 0, ULONG_MAX, 0, 0);
