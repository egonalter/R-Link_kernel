...
{
	int i;
	DHD_ERROR(("dumping data with length %d\n ", bus->varsz));
	for (i = 0; i < bus->varsz; i++) {
		if ((i) && ((i % 16) == 0)) {
			DHD_ERROR(("\n "));
		}
		DHD_ERROR(("%02x ", bus->vars[i]));
	}
	for (i = 0; i < bus->varsz; i++) {
		if ((i) && ((i % 16) == 0)) {
			DHD_ERROR(("\n"));
		}
		DHD_ERROR(("%c", bus->vars[i] >=0x20 ? bus->vars[i] : ' '));
	}
}
#endif
...
#if 0	
{
	int i;
	DHD_ERROR(("dumping data with length %d\n ", bus->varsz));
	for (i = 0; i < bus->varsz; i += 16) {
		DHD_ERROR(("%02x %02x %02x %02x  %02x %02x %02x %02x  %02x %02x %02x %02x  %02x %02x %02x %02x\n", 
			bus->vars[i],   bus->vars[i+1], bus->vars[i+2], bus->vars[i+3],
			bus->vars[i+4], bus->vars[i+5], bus->vars[i+6], bus->vars[i+7],
			bus->vars[i+8], bus->vars[i+9], bus->vars[i+10], bus->vars[i+11],
			bus->vars[i+12], bus->vars[i+13], bus->vars[i+14], bus->vars[i+15]));
		DHD_ERROR((" %c  %c  %c  %c   %c  %c  %c  %c   %c  %c  %c  %c   %c  %c  %c  %c \n", 
			bus->vars[i+0 ] >=0x20 ? bus->vars[i+0 ] : ' ', bus->vars[i+1 ] >=0x20 ? bus->vars[i+1 ] : ' ',
			bus->vars[i+2 ] >=0x20 ? bus->vars[i+2 ] : ' ', bus->vars[i+3 ] >=0x20 ? bus->vars[i+3 ] : ' ',
			bus->vars[i+4 ] >=0x20 ? bus->vars[i+4 ] : ' ', bus->vars[i+5 ] >=0x20 ? bus->vars[i+5 ] : ' ',
			bus->vars[i+6 ] >=0x20 ? bus->vars[i+6 ] : ' ', bus->vars[i+7 ] >=0x20 ? bus->vars[i+7 ] : ' ',
			bus->vars[i+8 ] >=0x20 ? bus->vars[i+8 ] : ' ', bus->vars[i+9 ] >=0x20 ? bus->vars[i+9 ] : ' ',
			bus->vars[i+10] >=0x20 ? bus->vars[i+10] : ' ', bus->vars[i+11] >=0x20 ? bus->vars[i+11] : ' ',
			bus->vars[i+12] >=0x20 ? bus->vars[i+12] : ' ', bus->vars[i+13] >=0x20 ? bus->vars[i+13] : ' ',
			bus->vars[i+14] >=0x20 ? bus->vars[i+14] : ' ', bus->vars[i+15] >=0x20 ? bus->vars[i+15] : ' '));
	}
}
#endif
