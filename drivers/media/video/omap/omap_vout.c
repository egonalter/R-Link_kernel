...
#define QQVGA_HEIGHT		120
 ...
 #define OMAP_VOUT_VRFB_MAX_BUF_SIZE	(VID_MAX_WIDTH*VID_MAX_HEIGHT*4)
...
static u32 video_vrfb_bufsize = OMAP_VOUT_VRFB_MAX_BUF_SIZE;
...
module_param(video_vrfb_bufsize, uint, S_IRUGO);
MODULE_PARM_DESC(video_vrfb_bufsize,
	"Size of the buffer to be allocated for video VRFB");
...
MODULE_PARM_DESC(video3_bufsize, "Size of the buffer to be allocated for \
		video3 device");
#endif
...
static unsigned int get_vout_bufsize(struct omap_vout_device *vout)
{
	if (vout->vid == OMAP_VIDEO1)
		return video1_bufsize;
	if (vout->vid == OMAP_VIDEO2)
		return video2_bufsize;
#ifdef CONFIG_ARCH_OMAP4
	if (vout->vid == OMAP_VIDEO3)
		return video3_bufsize;
#endif
	BUG();
}

static unsigned int get_vout_numbuffers(struct omap_vout_device *vout)
{
	if (vout->vid == OMAP_VIDEO1)
...
static unsigned int get_vrfb_numbuffers(struct omap_vout_device *vout)
{
	if (vout->vid == OMAP_VIDEO1)
		return vid1_static_vrfb_alloc ?	video1_numbuffers : 0;
	if (vout->vid == OMAP_VIDEO2)
		return vid2_static_vrfb_alloc ?	video2_numbuffers : 0;
	BUG();
}
...
/* Allocate buffers */
static unsigned long omap_vout_alloc_buffer(size_t buf_size, unsigned long *phys_addr)
{
	void *addr, *virt_addr;
	size_t size;

	size = PAGE_ALIGN(buf_size);
	virt_addr = alloc_pages_exact(size, GFP_KERNEL | GFP_DMA | __GFP_NOWARN);
	if (!virt_addr) {
		printk(KERN_ERR "omap_vout_alloc_buffer - unable to allocate pages for %u bytes\n", PAGE_ALIGN(buf_size));
		return 0;
	}
	...
	*phys_addr = virt_to_phys(virt_addr);
	dbg("phys_addr: %lx, size: %d\n", *phys_addr, PAGE_ALIGN(buf_size));
	return (unsigned long) virt_addr;
}
#endif

/* Free buffers */
static void omap_vout_free_buffer(unsigned long virtaddr, unsigned long phys_addr, size_t buf_size)
{
		...
		unsigned int *count, int startindex)
{
	...
	dbg("count: %u, size: %d\n", *count - (startindex == -1 ? 0 : startindex), vout->smsshado_size);
...
static void omap_vout_free_buffers(struct omap_vout_device *vout)
{
	...
	numbuffers = get_vout_numbuffers(vout);
	vout->buffer_size = get_vout_bufsize(vout);
...
#ifndef CONFIG_ARCH_OMAP4
/* Free VRFB buffers allocated at init time */
static void omap_vout_free_vrfb_buffers(struct omap_vout_device *vout)
{
	int j, numbuffers;

	numbuffers = get_vrfb_numbuffers(vout);

	for (j = 0; j < numbuffers; j++) {
		omap_vout_free_buffer(vout->smsshado_virt_addr[j],
			vout->smsshado_phy_addr[j], vout->smsshado_size);
		vout->smsshado_virt_addr[j] = 0;
		vout->smsshado_phy_addr[j] = 0;
	}
}
			  ...
			  unsigned int *size)
{
	...
	/* :TODO: ??? */
	startindex = get_vout_numbuffers(vout);
	...
	*size = PAGE_ALIGN(*size);
	...
	dbg("count: %u, size: %d\n", *count - startindex, vout->buffer_size);
...
static void omap_vout_free_allbuffers(struct omap_vout_device *vout)
{
	...
	num_buffers = get_vout_numbuffers(vout);
	...
	/* Free only the VRFB buffers allocated during reqbufs */
	i = get_vrfb_numbuffers(vout);
	for (; i < vout->buffer_allocated; i++) {
		omap_vout_free_buffer(vout->smsshado_virt_addr[i],
			vout->smsshado_phy_addr[i], vout->smsshado_size);
...
static int omap_vout_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	while (size > 0) {
		...
		if (vm_insert_page(vma, start, cpage)) {
			printk(KERN_ERR "vout_mmap: Failed to insert bus_addr %x page to VMA\n", dmabuf->bus_addr);
			return -EAGAIN;
			}
		start += PAGE_SIZE;
		pos += PAGE_SIZE;
		...
		int vid_num)
{
	...
	int i, j, r = 0;
	unsigned int numbuffers, vrfb_numbufers;
#endif
...
#ifndef TILER_ALLOCATE_V4L2
	numbuffers = get_vout_numbuffers(vout);
	vrfb_numbufers = get_vrfb_numbuffers(vout);
	vout->buffer_size = get_vout_bufsize(vout);

	dbg("count: %u, size: %d\n", numbuffers, vout->buffer_size);

	for (i = 0; i < numbuffers; i++) {
		vout->buf_virt_addr[i] =
			omap_vout_alloc_buffer(vout->buffer_size, &vout->buf_phy_addr[i]);
	...
	/* Note that since the tiler is disabled we don't necesarily i
	   need to have a buffer that is aligned to a TILE */
	vout->smsshado_size = PAGE_ALIGN(video_vrfb_bufsize);
	...
	if (omap_vout_allocate_vrfb_buffers(vout, &vrfb_numbufers, -1)) {
		r =  -ENOMEM;
		goto free_buffers;
	}
	return 0;
...
#endif
#else
	/* Free the VRFB buffer allocated at init time */
