...
static irqreturn_t isp_isr(int irq, void *_pdev)
{
	...
	// this does not seem to work well with BT656
	if (isp->pipeline.out_pix.field != V4L2_FIELD_INTERLACED) {
		if (irqstatus & CCDC_VD1) {
	...
	if (irqstatus & CCDC_VD0) {
		if (isp->pipeline.out_pix.field == V4L2_FIELD_INTERLACED) {
	 		if (isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC, ISPCCDC_SYN_MODE) & ISPCCDC_SYN_MODE_FLDSTAT) {
				isp->current_field = 1;
			}
			else {
				isp->current_field = 0;
			    ...
			    enum isp_interface_type sensor_isp_if)
{
	...
	if (pipe->modules & OMAP_ISP_CCDC) {
		...
		/* We are only passing the even fields - halve the picture height 
		 * to reflect that */
		if (pix_output->field == V4L2_FIELD_INTERLACED) {
			pix_output->height = pipe->ccdc_out_h >> 1;
			pix_output->sizeimage = pix_output->bytesperline * 
						pix_output->height;
		}
#endif
	}
			  ...
			  enum isp_interface_type sensor_isp_if)
{
	...
	// Set pipeline's CROP parameters before applying to CCDC
	pipe.ccdc_in_h_st = isp->config->crop.left;
	pipe.ccdc_in_v_st = isp->config->crop.top;
...
static void isp_buf_init(struct device *dev)
{
	...
	bufs->queue = 0;
	bufs->done = 0;
	bufs->wait_hs_vs = isp->config ? isp->config->wait_hs_vs : 0;
...
static void isp_buf_process(struct device *dev, struct isp_bufs *bufs)
{
	...
	if (!last) {
		...
		// Re-start CCDC if needed
		if (! (isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC, ISPCCDC_PCR) &  ISPCCDC_PCR_EN) ) {
			if (CCDC_CAPTURE(isp)) {	
				ispccdc_enable(&isp->isp_ccdc, 1);
			}
		}
	...
	DPRINTK_ISPCTRL(KERN_ALERT "%s: finish %d mmu %p\n", __func__,
			...
			(void *) (bufs->buf+((bufs->done - 1 + NUM_BUFS)
				    % NUM_BUFS))->isp_addr);
