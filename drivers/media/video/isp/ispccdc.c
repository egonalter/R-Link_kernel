				   ...
				   struct isp_pipeline *pipe)
{
	...
	case CCDC_YUV_SYNC:
		...
		syncif.datapol = 0;
		syncif.datsz = DAT8;
		if (pipe->out_pix.field == V4L2_FIELD_INTERLACED) {
			syncif.fldmode = 1;
		}
		else {
			syncif.fldmode = 0;
		       ...
		       struct isp_pipeline *pipe)
{
	...
	isp_reg_writel(dev,
      ...
      if (pipe->out_pix.field == V4L2_FIELD_INTERLACED) {
		isp_reg_writel(dev,
			...
			pipe->ccdc_in_v_st << ISPCCDC_VERT_START_SLV1_SHIFT,
			OMAP3_ISP_IOMEM_CCDC,
			ISPCCDC_VERT_START);
	}
	else {
	...
	if (pipe->out_pix.field == V4L2_FIELD_INTERLACED) {
		vert_lines = (pipe->ccdc_out_h >> 1) - 1;
		vdint0 = vert_lines;
	}
	else {
		vert_lines = pipe->ccdc_out_h - 1;
		vdint0 = pipe->ccdc_out_h - 2;
	}
		
	isp_reg_writel(dev, vert_lines <<
...
#ifdef CONFIG_VIDEO_OMAP3_SKIP_EVEN_FIELDS
	/* ISP does not re-combine ODD & EVEN fields */
...
/*
	if (! printed_out)
	{
	printk(KERN_INFO "###CCDC PCR=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_PCR));
	printk(KERN_INFO "###ISP_CTRL =0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_MAIN,
				      ISP_CTRL));
	printk(KERN_INFO "###ISP_IRQ0ENABLE in ccdc =0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_MAIN,
				      ISP_IRQ0ENABLE));
	printk(KERN_INFO "###ISP_IRQ0STATUS in ccdc =0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_MAIN,
				      ISP_IRQ0STATUS));
	printk(KERN_INFO "###CCDC SYN_MODE=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_SYN_MODE));
	printk(KERN_INFO "###CCDC HORZ_INFO=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_HORZ_INFO));
	printk(KERN_INFO "###CCDC VERT_START=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_VERT_START));
	printk(KERN_INFO "###CCDC VERT_LINES=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_VERT_LINES));
	printk(KERN_INFO "###CCDC CULLING=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_CULLING));
	printk(KERN_INFO "###CCDC HSIZE_OFF=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_HSIZE_OFF));
	printk(KERN_INFO "###CCDC SDOFST=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_SDOFST));
	printk(KERN_INFO "###CCDC SDR_ADDR=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_SDR_ADDR));
	printk(KERN_INFO "###CCDC CLAMP=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_CLAMP));
	printk(KERN_INFO "###CCDC COLPTN=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_COLPTN));
	printk(KERN_INFO "###CCDC VP_OUT=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_VP_OUT));
	printk(KERN_INFO "###CCDC VDINT=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_VDINT));
	printk(KERN_INFO "###CCDC_SDR_ADDR= 0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_SDR_ADDR));
	printk(KERN_INFO "###CCDC FMTCFG=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_FMTCFG));
	printk(KERN_INFO "###CCDC FMT_HORZ=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_FMT_HORZ));
	printk(KERN_INFO "###CCDC FMT_VERT=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_FMT_VERT));
	printk(KERN_INFO "###CCDC LSC_CONFIG=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_LSC_CONFIG));
	printk(KERN_INFO "###CCDC LSC_INIT=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_LSC_INITIAL));
	printk(KERN_INFO "###CCDC LSC_TABLE BASE=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_LSC_TABLE_BASE));
	printk(KERN_INFO "###CCDC LSC TABLE OFFSET=0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_LSC_TABLE_OFFSET));

	printk(KERN_INFO "###CCDC REC656IF =0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_REC656IF));
	printk(KERN_INFO "###CCDC CFG =0x%x\n",
			isp_reg_readl(dev, OMAP3_ISP_IOMEM_CCDC,
				      ISPCCDC_CFG));
	printed_out = 1;
	}
