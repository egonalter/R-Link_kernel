...
struct fimc_limit fimc_limits[FIMC_DEVICES] = {
	{
		.pre_dst_w	= 4224,
		.bypass_w	= 8192,
		.trg_h_no_rot	= 4224,     // horizontal pixel number of target image (without output roatation)
		.trg_h_rot	= 1920,        // horizontal pixel number of target image (with output rotation)
		.real_w_no_rot	= 8192,    // input dma real image horizontal pixel size (without input rotation)
		.real_h_rot	= 1920,       // input dma real image vertical pixel size (with input rotation)
	}, {
		.pre_dst_w	= 4224,
		.bypass_w	= 8192,
		.trg_h_no_rot	= 4224,
...
/* Tushar - Code migration from C100 */
				...
				struct v4l2_rect *crop)
{
	...
	case V4L2_PIX_FMT_RGB32:
		if (0x50 == pdata->hw_ver || 0x45 == pdata->hw_ver) 
			cfg_y |= S3C_CIOYOFF_HORIZONTAL(crop->left);
		else {
			// added by jamie (2009.12.09)
			// check condition
			if (1 == (crop->left%2))
				cfg_y |= S3C_CIOYOFF_HORIZONTAL((crop->left+1)*4);
			else
	...
	case V4L2_PIX_FMT_RGB565:
		...
		if (0x50 == pdata->hw_ver || 0x45 == pdata->hw_ver)
			...
			if (1 == (crop->left%2))
				cfg_y |= S3C_CIOYOFF_HORIZONTAL((crop->left+1)*2);
			else
	...
	case V4L2_PIX_FMT_NV61:
		cfg_y |= S3C_CIOYOFF_HORIZONTAL(crop->left);
		cfg_y |= S3C_CIOYOFF_VERTICAL(crop->top);
		if (pdata->hw_ver == 0x50 || 0x45 == pdata->hw_ver) {
	...
	if (crop->left || crop->top ||
		...
		case V4L2_PIX_FMT_NV61:		
			...
			if (pdata->hw_ver == 0x50 || 0x45 == pdata->hw_ver) {
				...
				cfg_cb |= S3C_CIICBOFF_HORIZONTAL(crop->left/2);
		...
		case V4L2_PIX_FMT_NV21:
			...
			if (pdata->hw_ver == 0x50 || 0x45 == pdata->hw_ver){
				...
				cfg_cb |= S3C_CIICBOFF_HORIZONTAL(crop->left/4);
				cfg_cb |= S3C_CIICBOFF_VERTICAL(crop->top/4);
			}
			break;
		...
		case V4L2_PIX_FMT_YUV422P:
			...
			if (pdata->hw_ver == 0x50 || 0x45 == pdata->hw_ver) {
				...
				cfg_cb |= S3C_CIICBOFF_HORIZONTAL(crop->left / 2);
				cfg_cb |= S3C_CIICBOFF_VERTICAL(crop->top / 2);
				cfg_cr |= S3C_CIICROFF_HORIZONTAL(crop->left / 2);
		...
		case V4L2_PIX_FMT_YUV420:
			...
			if (pdata->hw_ver == 0x50 || 0x45 == pdata->hw_ver) {
				...
				cfg_cb |= S3C_CIICBOFF_HORIZONTAL(crop->left / 4);
				cfg_cb |= S3C_CIICBOFF_VERTICAL(crop->top / 4);
				cfg_cr |= S3C_CIICROFF_HORIZONTAL(crop->left / 4);
				cfg_cr |= S3C_CIICROFF_VERTICAL(crop->top / 4);
			}
			break;
...
void fimc_dump_regs(struct fimc_control *ctrl)
{
	int i = 0;

	for(i = 0; i < 0x194; i+=4){
		printk("[FIMC-%d][Reg-0x%08x][Value-0x%08x]\n", ctrl->id, i, readl(ctrl->regs + i));
	}
}
