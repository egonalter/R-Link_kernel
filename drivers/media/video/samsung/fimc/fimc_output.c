...
#if 0 //Kamat added to disable check
				       ...
				       int idx)
{
	...
	u32 c_size = PAGE_ALIGN(y_size >> 2);
	int ret = -1, i;
	...
	case V4L2_PIX_FMT_NV12T:
		buf_set.base[FIMC_ADDR_Y] = (dma_addr_t)ctx->fbuf.base;
		buf_set.base[FIMC_ADDR_CB] = buf_set.base[FIMC_ADDR_Y] + ALIGN(ALIGN(width,128) * ALIGN(height, 32), SZ_8K);
		break;
	default:
		fimc_err("%s: Invalid pixelformt : %d\n", __func__, format);
		return -EINVAL;
