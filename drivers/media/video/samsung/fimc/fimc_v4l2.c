...
#include "fimc.h"
static struct wake_lock fimc_delayed_work_wake_lock;
...
static int fimc_streamon(struct file *filp, void *fh, enum v4l2_buf_type i)
{
	...
	if (i == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		wake_lock_init(&fimc_delayed_work_wake_lock, WAKE_LOCK_SUSPEND, "fimc_delayed_work");
		wake_lock(&fimc_delayed_work_wake_lock);
		clk_enable(ctrl->clk);
...
static int fimc_streamoff(struct file *filp, void *fh, enum v4l2_buf_type i)
{
	...
	if (i == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		...
		wake_unlock(&fimc_delayed_work_wake_lock);
		wake_lock_destroy(&fimc_delayed_work_wake_lock);
	} else if (i == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
