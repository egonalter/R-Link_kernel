...
#ifdef CONFIG_CPU_FREQ
//	if (0 == ctrl->id)
//		s5pc110_set_cpufreq_level(RESTRICT_STATE);
#endif /* CONFIG_CPU_FREQ */
...
#ifdef CONFIG_CPU_FREQ
//	if (0 == ctrl->id)
//		s5pc110_set_cpufreq_level(NORMAL_STATE);
#endif /* CONFIG_CPU_FREQ */

	fimc_dbg("FIMC%d %d released.\n", ctrl->id, atomic_read(&ctrl->in_use));

	clk_disable(hclk_fimc);
...
static int __devinit fimc_probe(struct platform_device *pdev)
{
	...
	hclk_fimc = clk_get(NULL,"hclk_fimc");
	if (IS_ERR(srclk)) {
		printk( "%s: failed to get hclk fimc *********************************************************\n",
				__func__);
	}
//	pdata->hw_ver = 0x50;
	/* fimc source clock */
