...
#define 	ENABLE_CHECK_SEQ_HEADER					0
	...
	#define ENABLE_DEBUG_DEC_EXE_INTR_OK				0			// Make log about Normal Interrupts.
...
static void mfc_set_encode_init_param(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	switch (mfc_ctx->MfcCodecType)
	{
		...
		case MPEG4_ENC:
			...
			/* if in_RC_mb_enable is '1' */
#if	ENABLE_ENC_MB
			if(enc_init_mpeg4_arg->in_RC_frm_enable != 1)
				enc_init_h264_arg->in_RC_frm_enable = 1;
#endif
...
#if	ENABLE_ENC_MB
			if(enc_init_mpeg4_arg->in_RC_frm_enable != 1)
				enc_init_mpeg4_arg->in_RC_frm_enable = 1;
#endif			
...
MFC_ERROR_CODE mfc_init_encode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
   ...
   else
   {
	...
	stream_vir = phys_to_virt(context_addr);	
		
	
	dmac_inv_range((void *)stream_vir, (void *)(stream_vir + context_size));
	dmac_clean_range((void *)stream_vir, (void *)(stream_vir + context_size));
	flush_cache_all();
...
MFC_ERROR_CODE mfc_init_decode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	if(init_arg->in_strm_size < 5)
	{
		mfc_err("Input stream size invalid..... (%d)\n", init_arg->in_strm_size);	
	...
	if(context_size == 0)
	{
		mfc_info("context_size......................................................................... = %d/n",context_size);
...
#ifdef	ENABLE_DEBUG_DEC_EXE_INTR_ERR		
#if	ENABLE_DEBUG_DEC_EXE_INTR_ERR
		makefile_mfc_decinit_err_info(mfc_ctx, init_arg,nIntrRet);
#endif
#endif			
