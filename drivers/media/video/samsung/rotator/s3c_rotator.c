...
#elif defined(CONFIG_CPU_S5PC100) || defined(CONFIG_CPU_S5PV210)
irqreturn_t s3c_rotator_irq(int irq, void *dev_id)
{
	struct s3c_rotator_ctrl	*ctrl = &s3c_rot;
	unsigned int cfg;
...
int s3c_rotator_open(struct inode *inode, struct file *file)
{
	...
	params	= kmalloc(sizeof(ro_params), GFP_KERNEL);
	if (params == NULL) {
...
int s3c_rotator_probe(struct platform_device *pdev)
{
	...
	h_rot_mutex = kmalloc(sizeof(struct mutex), GFP_KERNEL);
	if (h_rot_mutex == NULL) {
