	...
	int max_uV)
{
	...
	val &= ~mask;
	ret = s5m8751_reg_write(s5m8751, volt_reg,  mV);
out:
	return ret;
}
...
static int s5m8751_regulator_set_suspend_disable(struct regulator_dev *rdev)
{
	...
	case S5M8751_LDO6:
		enable_reg = S5M8751_SLEEP_CNTL2;
		shift = S5M8751_LDO6_SHIFT;
		printk("SLEEP control LDO6  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %d       shift  %d\n",__LINE__,shift);
		break;
	case S5M8751_BUCK1_1:
	case S5M8751_BUCK1_2:
		enable_reg = S5M8751_SLEEP_CNTL2;
		shift = S5M8751_BUCK1_SHIFT;
		printk("SLEEP control  BUCK1 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %d       shift  %d\n",__LINE__,shift);
		break;
	case S5M8751_BUCK2_1:
	case S5M8751_BUCK2_2:
		enable_reg = S5M8751_SLEEP_CNTL2;
		shift = S5M8751_BUCK2_SHIFT;
		printk("SLEEP control  BUCK2 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %d       shift  %d\n",__LINE__,shift);
		break;
	default:
		return -EINVAL;
	}
