...
{
	struct s5m8751 *s5m8751 = get_irq_chip_data(irq);
	s5m8751_irqs[irq - s5m8751->irq_base].enable = s5m8751_irqs[irq - s5m8751->irq_base].offs;
}
				...
				struct s5m8751_pdata *pdata)
{
	...
	gpio_request(S5P6450_GPN(1),"GPN_8751");
	s3c_gpio_setpull(S5P6450_GPN(1), S3C_GPIO_PULL_NONE);
	gpio_free(S5P6450_GPN(1));
