...
#define twl_has_watchdog()        false
		...
		defined(CONFIG_SND_SOC_ABE_TWL6040)
#define twl_has_codec()	true
#else
#define twl_has_codec()	false
#endif

#if defined(CONFIG_SENSORS_TWL4030_HOTDIE) || defined(CONFIG_SENSORS_TWL4030_HOTDIE_MODULE)
#define twl_has_hotdie()	true
#else
#define twl_has_hotdie()	false
#endif
...
add_children(struct twl4030_platform_data *pdata, unsigned long features)
{
	...
	if (twl_has_madc() && pdata->madc) {
		child = add_child(2, "twl4030_madc",
				...
				false, pdata->irq_base + MADC_INTR_OFFSET, 0);
		if (IS_ERR(child))
			return PTR_ERR(child);
	}
	...
	if (twl_has_hotdie()) {
		child = add_child(1, "twl4030_hotdie", NULL, 0, true,
			...
			struct twl4030_clock_init_data *clock)
{
	...
	ctrl |= HIGH_PERF_SQ;
	if (clock && clock->high_perf_sq_disable)
		ctrl &= ~HIGH_PERF_SQ;

	if (clock && clock->clk_slicer_bypass)
		ctrl |= SLICER_BYPASS;
