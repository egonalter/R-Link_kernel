...
#define	OMAP_TLL_CHANNEL_CONF_ULPIOUTCLKMODE		(1 << 7)
#define	OMAP_TLL_CHANNEL_CONF_TLLFULLSPEED		(1 << 6)
#define	OMAP_TLL_CHANNEL_CONF_TLLATTACH			(1 << 4)
#define	OMAP_TLL_CHANNEL_CONF_UTMIISADEV		(1 << 3)
...
static int __devinit usbhs_omap_probe(struct platform_device *pdev)
{
	...
	omap->usbhost_hs_fck = clk_get(dev, "usbhost_120m_fck");
	...
	omap->usbhost_fs_fck = clk_get(dev, "usbhost_48m_fck");
...
static void usbhs_omap_tll_init(struct device *dev, u8 tll_channel_count)
{
	...
	for (i = 0; i < tll_channel_count; i++) {
		reg = (OMAP_TLL_CHANNEL_CONF_ULPI_ULPIAUTOIDLE |
			...
			OMAP_TLL_CHANNEL_CONF_ULPIOUTCLKMODE |
			OMAP_TLL_CHANNEL_CONF_TLLFULLSPEED |
			OMAP_TLL_CHANNEL_CONF_TLLATTACH |
			OMAP_TLL_CHANNEL_CONF_UTMIISADEV);			
...
static void usbhs_clock_power(struct device *dev, int on)
{
	struct usbhs_hcd_omap *omap = dev_get_drvdata(dev);
...
static int usbhs_enable(struct device *dev)
{
	...
	/* Wait for TLL reset to complete: do not read from the 
	   USBTLL_SYSSTATUS because this can occasionally lead to data aborts */
	udelay(100);
...
static void usbhs_disable(struct device *dev)
{
	...
	/* Wait for TLL operation to complete: do not read from the
	   USBTLL_SYSSTATUS because this can occasionally lead to data aborts */
	udelay(100);
	...
	if (pdata->ehci_data->phy_reset) {
		if (gpio_is_valid(pdata->ehci_data->reset_gpio_port[0])) {
			gpio_set_value(pdata->ehci_data->reset_gpio_port[0], 0);
			gpio_free(pdata->ehci_data->reset_gpio_port[0]);
		}

		if (gpio_is_valid(pdata->ehci_data->reset_gpio_port[1])) {
			gpio_set_value(pdata->ehci_data->reset_gpio_port[1], 0);
			gpio_free(pdata->ehci_data->reset_gpio_port[1]);
		}
	}
	return;
...
int omap_usbhs_suspend(struct device *dev)
{
	return usbhs_suspend(dev->parent);
}
EXPORT_SYMBOL_GPL(omap_usbhs_suspend);

int omap_usbhs_resume(struct device *dev)
{
	return usbhs_resume(dev->parent);
}
EXPORT_SYMBOL_GPL(omap_usbhs_resume);
