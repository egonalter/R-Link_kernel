...
/* For iPod communication large amount of data is received for ArtWorks.
 * Some iPods send 320x320 pictures => size: 320*320*2 = 204800 bytes
 * Since HIDDEV_BUFFER_SIZE must be a power of 2 it needs to be 262144
 * Unfortunately this requires significant amount of memory.
 * For every payload byte a hiddev_usage_ref (24 bytes) needs to be allocated.
 * So in this case memory requirement will exceed 6 Mb...:-(    */
#define HIDDEV_BUFFER_SIZE	262144 // 0x40000
#else
...
static int hiddev_open(struct inode *inode, struct file *file)
{
	...
	if (!(list = vmalloc(sizeof(struct hiddev_list))))
		return -ENOMEM;
