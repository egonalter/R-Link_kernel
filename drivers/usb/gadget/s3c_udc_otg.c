...
extern struct platform_driver s3c6410_otg_driver;
#endif

#if defined(CONFIG_S5P6450_S5M8752) || defined(CONFIG_S5P6450_S5M8751)
static struct wake_lock cablepresent;
static int lock_status = 0;
void s3c_cable_check_status(int flag)
{
	
	if(flag){	
		wake_lock_init(&cablepresent, WAKE_LOCK_SUSPEND, "cable_present");
		wake_lock(&cablepresent);
		lock_status =  1;
	}else if(lock_status){
		wake_unlock(&cablepresent);
		wake_lock_destroy(&cablepresent);
		lock_status = 0 ;
	}
...
#ifdef CONFIG_TOMTOM_USB_MODE_LITE
int s3c_udc_enable(struct usb_gadget *gadget, int enable)
{
	unsigned long flags;
	struct s3c_udc *dev = the_controller;
	u32 dev_gctl;
	...
	DEBUG("udc->enabled=%d,=%d\n", dev->udc_state, enable);
...
#ifdef CONFIG_TOMTOM_USB_MODE_LITE
	dev->enabled = 0;
	dev->otg_mode = -1; /* -1 means that it is not ready. */
...
static int s3c_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (dev->driver) {
		if (dev->driver->suspend){
			printk("calling dev->driver->suspend\n");
			dev->driver->disconnect(&dev->gadget);
...
static int s3c_udc_resume(struct platform_device *pdev)
{
	...
	if (dev->driver) {
		...
		if (dev->driver->resume){
			printk("calling dev->driver->resume\n");		
 ...
 * changes usb mode as provided in the argument of the function.
 * Currently we work with just two modes as below
 * USB MODES
 *		  -> OTG_DEVICE_MODE : disable otg host and enable udc
 *		  -> OTG_HOST_MODE : disable udc and enable otg host
 * Return value : 0 (success), -1 (Failed)
 * Written by SoonYong, Cho (Tue 16, Nov 2010)
 * Modified by Praveen Paneri (Mon Oct 3, 2011)
 */
int s3c_change_usb_mode(int mode)
{
	struct s3c_udc *dev = the_controller;
	int retval = 0;
	...
	switch (mode) {
	case OTG_DEVICE_MODE:
		if (dev->otg_mode == OTG_HOST_MODE) {
		...
		if (retval != 0) {
			printk(KERN_ERR "otg device can't get irq %i, err %d\n",
							IRQ_OTG, retval);
			mutex_unlock(&dev->mutex);
			return -1;
		}
		dev->otg_mode = OTG_DEVICE_MODE;
		/* re-enable UDC */
		retval = s3c_udc_enable(NULL, 1);
		break;

	case OTG_HOST_MODE:
		/* Disable UDC */
		if (dev->enabled == 0) {
			s3c_udc_enable(NULL, 1);
			dev->gadget.speed = USB_SPEED_HIGH;
		}
		retval = s3c_udc_enable(NULL, 0);
		/* Enable host */
		if (dev->otg_mode != OTG_HOST_MODE) {
			free_irq(IRQ_OTG, dev);
			if (platform_driver_register(&s3c6410_otg_driver) < 0) {
				printk(KERN_ERR "otg host : Fail register\n");
				dev->otg_mode =	OTG_DEVICE_MODE;
				mutex_unlock(&dev->mutex);
				return -1;
			} else {
				DEBUG("otg host : registered \n");
				dev->otg_mode = OTG_HOST_MODE;
			}
		} else
			DEBUG("Already in host mode, please check status.\n");
		break;
	}
	mutex_unlock(&dev->mutex);
	return 0;
}
EXPORT_SYMBOL(s3c_change_usb_mode);
#endif /* CONFIG_TOMTOM_USB_MODE_LITE */
