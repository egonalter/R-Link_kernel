...
#ifndef USE_DMA_CACHE_MAINT
void ___dma_single_cpu_to_dev(const void *kaddr, size_t size,
        enum dma_data_direction dir);
#endif

//#if defined (CONFIG_MACH_SMDK6450) || defined (CONFIG_MACH_SMDKV210) || defined (CONFIG_MACH_VALDEZ)
...
#ifdef USE_DMA_CACHE_MAINT
	dma_cache_maint(buf, length, DMA_FROM_DEVICE);
#else
	___dma_single_cpu_to_dev(buf, length, DMA_FROM_DEVICE);
#endif
...
#else
	___dma_single_cpu_to_dev(buf, length, DMA_TO_DEVICE);
#endif
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
	...
	DEBUG_ISR("\n*** %s : GINTSTS=0x%x(on state %d), GINTMSK : 0x%x, DAINT : 0x%x, DAINTMSK : 0x%x\n",
			__func__, intr_status, dev->ep0state, gintmsk,
	...
	if (intr_status & INT_SUSPEND) {
		...
		/*this is used to take a wakelock on usb connection*/
	...
	if (intr_status & INT_RESET) {
		...
		if ((usb_status & 0xc0000) == (0x3 << 18)) {
			...
			usb_mode_lite_handle_usb_reset();
#endif
			if (reset_available) {
...
#else
	___dma_single_cpu_to_dev(usb_ctrl, bytes, DMA_FROM_DEVICE);
#endif
...
static inline void set_test_mode(struct s3c_ep *ep)
{
	u32 ep_ctrl, dctl;
