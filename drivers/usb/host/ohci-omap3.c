...
#ifdef CONFIG_PM
/*-------------------------------------------------------------------------*/
static int ohci_omap_dev_suspend(struct device *dev)
{
	return omap_usbhs_suspend(dev);
}

static int ohci_omap_dev_resume(struct device *dev)
{
	return omap_usbhs_resume(dev);
}

static const struct dev_pm_ops ohci_omap_dev_pm_ops = {
	.suspend	= ohci_omap_dev_suspend,
	.resume_noirq	= ohci_omap_dev_resume,
};
#define OHCI_OMAP_DEV_PM_OPS (&ohci_omap_dev_pm_ops)
#else
