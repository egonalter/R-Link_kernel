...
*/
	g_pUsbHcd = usb_create_hcd(&s3c6410_otg_hc_driver, &pdev->dev, pdev->dev.bus->name);
	if (g_pUsbHcd == NULL) 
	{
		ret_val = -ENOMEM;
...
static int s3c_otg_suspend(struct platform_device *dev, pm_message_t state)
{
	printk("otg host suspend\n");

	return 0;
}

static int s3c_otg_resume(struct platform_device *dev)
{
	printk("otg host resume\n");
	return 0;
}
#else
 ...
 */
struct platform_driver s3c6410_otg_driver = {
