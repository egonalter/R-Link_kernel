...
int	dec_perio_bus_time(u32 bus_time)
{
	...
	{
		perio_used_bustime = perio_used_bustime - bus_time;
		return USB_ERR_SUCCESS;
	}
	else
	{
...
start_sched_perio_transfer:
		...
		if(err_sched==USB_ERR_SUCCESS)
		{
			...
			if((!td->is_transferring) && (!td->is_transfer_done))
			{
				...
				{
					if (scheduling_ed->ed_desc.endpoint_type == ISOCH_TRANSFER)
						scheduling_ed->ed_desc.sched_frame = (oci_get_frame_num() + 
							scheduling_ed->ed_desc.interval) & HFNUM_MAX_FRNUM;
