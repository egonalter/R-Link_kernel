...
{ 
	...
	ed_t	*target_ed = NULL;
	isoch_packet_desc_t *new_isoch_packet_desc = urb->iso_frame_desc; //  NULL;
	unsigned long	spin_lock_flag = 0;
	...
	if(!(urb->ep->hcpriv))
	{
		...
		f_is_ep_in = urb->transfer_flags & URB_DIR_MASK;
	...
	target_ed->ed_desc.is_ep_in = urb->transfer_flags & URB_DIR_MASK;
