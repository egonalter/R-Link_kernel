				...
				td_t 	*parent_td)
{
	...
	{
		urb_p = (struct urb *)parent_td->td_private;
		
		parent_td->cur_stransfer.buf_size = urb_p->iso_frame_desc[0].length;
		parent_td->cur_stransfer.start_phy_buf_addr = parent_td->phy_buf_addr + urb_p->iso_frame_desc[0].offset;
		parent_td->cur_stransfer.start_vir_buf_addr = parent_td->vir_buf_addr + urb_p->iso_frame_desc[0].offset;
	}
	else
	{
...
void  		update_perio_stransfer(td_t *parent_td)
{
	...
	switch(parent_td->parent_ed_p->ed_desc.endpoint_type)
	{
		...
		case ISOCH_TRANSFER:
			...
			parent_td->cur_stransfer.buf_size = urb_p->iso_frame_desc[parent_td->isoch_packet_index].length;
	                parent_td->cur_stransfer.start_phy_buf_addr = parent_td->phy_buf_addr + urb_p->iso_frame_desc[parent_td->isoch_packet_index].offset;
        	        parent_td->cur_stransfer.start_vir_buf_addr = parent_td->vir_buf_addr + urb_p->iso_frame_desc[parent_td->isoch_packet_index].offset;
			break;		
