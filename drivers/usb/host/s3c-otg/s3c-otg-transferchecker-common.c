	...
	while(do_try_cnt<MAX_CH_NUMBER)
	{
		...
		switch(done_td->parent_ed_p->ed_desc.endpoint_type)
		{
			...
			case INT_TRANSFER:
				...
				break;
			case ISOCH_TRANSFER:
				proc_result = process_iso_transfer(done_td, &ch_info);
				break;
			default:break;		
		}
		...
		{	
			...
			deallocate(done_td);
			if (done_td->parent_ed_p->num_td)
				do_schedule();

			if (done_td->parent_ed_p->ed_desc.endpoint_type != ISOCH_TRANSFER) 
			{	
				/* For ISOC, the tasklet will do this */
