...
#if defined(CONFIG_TOMTOM_PND)
/* In module TWL4030_MODULE_MAIN_CHARGE */
#define TPS65921_USB_DTCT_CTRL		0x02
#define TPS65921_USB_BCIICHG1		0x0a
#define TPS65921_USB_BCIICHG2		0x09
#define TPS65921_USB_BCIMFSTS3		0x0f
#define TPS65921_USB_BCIMFSTS4		0x10
#define TPS65921_USB_BCIMFTH8		0x1d
#define TPS65921_USB_BCC_CTRL		0x36
#define TPS65921_USB_BCC_CTRL2		0x37
#define TPS65921_USB_BCC_STS		0x38
#define TPS65921_USB_CHRG_CTRL1		0x39
#define TPS65921_USB_CHRG_CTRL2		0x3a

#define TPS65921_USB_BCC_INIT2		0x13
#define TPS65921_SW_USB_DET_EN		0x10
#define TPS65921_SW_VACCHRG_EN		(1<<4)

#define TPS65921_USB_CHG_DET_EN_SW	(1 << 7)
#define TPS65921_USB_DET_STS_MASK	(1 << 0)
#define TPS65921_USB_DET_STS_100MA	(1 << 0)
#define TPS65921_USB_DET_STS_500MA	(1 << 1)
#define TPS65921_USB_CHRG_DET_100MA     (0 << 3)
#define TPS65921_USB_CHRG_DET_500MA     (1 << 3)
...
struct twl4030_usb {
	...
	struct early_suspend	early_handler;
};

static void twl4030_usb_early_suspend (struct early_suspend *h);
static void twl4030_usb_early_resume (struct early_suspend *h);
...
static void twl4030_phy_power(struct twl4030_usb *twl, int on)
{
	...
	if (on) {
		...
		regulator_set_mode(twl->usb3v1, REGULATOR_MODE_NORMAL);
		...
		/* 
		 * Do not disable USB3V1 - put it into SLEEP mode to prevent 
		 * leakage in MADC and USB 
		*/
		regulator_set_mode(twl->usb3v1, REGULATOR_MODE_STANDBY);
	}
}
...
static void twl4030_phy_resume(struct twl4030_usb *twl)
{
	...
	/* Do not enable LDOs during boot if no cable attached */
...
#if defined(CONFIG_TOMTOM_PND)
	twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0x18, VUSB_DEDICATED1);
#else
...
static irqreturn_t twl4030_usb_irq(int irq, void *_twl)
{
	...
	if (status != USB_LINK_UNKNOWN) {
		...
		twl_i2c_write_u8(TWL4030_MODULE_MAIN_CHARGE,
					twl4030_readb(twl, TWL4030_MODULE_MAIN_CHARGE, TPS65921_USB_BCC_CTRL2) |
					TPS65921_SW_USB_DET_EN,
					TPS65921_USB_BCC_CTRL2);

		if (USB_LINK_VBUS == status) {
			u8 bcc_ctrl2, crg_ctrl2, crg_ctrl1;
			int limit;
			...
			bcc_ctrl2 = twl4030_readb(twl, TWL4030_MODULE_MAIN_CHARGE, TPS65921_USB_BCC_CTRL2);
			crg_ctrl1 = twl4030_readb(twl, TWL4030_MODULE_MAIN_CHARGE, TPS65921_USB_CHRG_CTRL1);
			crg_ctrl2 = twl4030_readb(twl, TWL4030_MODULE_MAIN_CHARGE, TPS65921_USB_CHRG_CTRL2);

			/* if usb is connected to usb host 500ma, otherwise 1500ma limit */
			if ((bcc_ctrl2 & TPS65921_SW_VACCHRG_EN) && !(crg_ctrl1 & TPS65921_USB_DET_STS_MASK)) {
				if (crg_ctrl2 != TPS65921_USB_DET_STS_500MA) {
					limit = 500000;
					bcc_ctrl2 &= ~TPS65921_USB_CHRG_DET_100MA;
				} else {
					limit = 1500000;
					bcc_ctrl2 |= TPS65921_USB_CHRG_DET_500MA;
				}

				twl_i2c_write_u8(TWL4030_MODULE_MAIN_CHARGE,
						 bcc_ctrl2, TPS65921_USB_BCC_CTRL2);

				dev_dbg(twl->dev, "Set  USB Charger limit to %duA\n", limit);
			} else {
				dev_dbg(twl->dev, "Push USB Cable AGAIN !!!\n");
			}
		} else {
...
static int __devinit twl4030_usb_probe(struct platform_device *pdev)
{
	...
	twl->early_handler.suspend = twl4030_usb_early_suspend;
	twl->early_handler.resume  = twl4030_usb_early_resume;
	register_early_suspend(&twl->early_handler);
...
static int __exit twl4030_usb_remove(struct platform_device *pdev)
{
	...
	unregister_early_suspend(&twl->early_handler);
...
static void twl4030_usb_early_suspend (struct early_suspend *h)
{
	...
	twl = container_of(h, struct twl4030_usb, early_handler);
...
static void twl4030_usb_early_resume (struct early_suspend *h)
{
	...
	status = request_irq(twl->irq, twl4030_usb_irq,
			...
			"twl4030_usb", twl);
	if (status < 0) {
		dev_dbg(twl->dev, "can't get IRQ %d, err %d\n",
			twl->irq, status);
	}
	twl->vbus_on = false;
	twl4030_usb_irq(twl->irq, twl);
