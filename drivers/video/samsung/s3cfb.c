...
int fb_is_primary_device(struct fb_info *fb)
{
	...
	printk("\n [fb%d] checking for primary device\n", win->id);
...
static int s3cfb_map_video_memory(struct fb_info *fb)
{
	...
	if (!fb->screen_base)
		...
		printk("[fb%d] dma: 0x%08x, cpu: 0x%08x, size: 0x%08x\n", \
...
static int s3cfb_unmap_video_memory(struct fb_info *fb)
{
	...
	if (fix->smem_start) {
		...
		fix->smem_start = 0;
		fix->smem_len = 0;
		printk("[fb%d] video memory released\n", win->id);
	}
...
static int s3cfb_blank(int blank_mode, struct fb_info *fb)
{
	...
	case FB_BLANK_UNBLANK:
		if (fb->fix.smem_start)
			...
			printk("[fb%d] no allocated memory for unblank\n", \
				win->id);
		break;
...
static int s3cfb_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(fbdev->clock)) {
		printk("failed to get fimd clock source\n");
		ret = -EINVAL;
		goto err_clk;
	}
	...
	if (pdata->reset_lcd_onoff)
		pdata->reset_lcd_onoff(pdev,1);
...
#if 0 //Moving LDI init code below
...
static struct platform_driver s3cfb_driver = {
	.probe		= s3cfb_probe,
	.remove		= s3cfb_remove,
#ifndef CONFIG_HAS_EARLYSUSPEND // mod by ksoo (0907)
