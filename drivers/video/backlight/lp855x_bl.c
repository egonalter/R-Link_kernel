...
static int temp_trace_secs;
module_param(temp_trace_secs, int, 0644);
static void temp_delayed_work(struct work_struct *work);
...
static int lp855x_bl_update_status(struct backlight_device *bl)
{
	...
	} else if (!lp->running_work)
		temp_delayed_work(&lp->work.work);
...
static struct backlight_ops lp855x_bl_ops = {
	.options = BL_CORE_SUSPENDRESUME,
	.update_status = lp855x_bl_update_status,
	.get_brightness = lp855x_bl_get_brightness,
};

static int lp855x_get_temperature(struct lp855x *lp)
{
	int temp;
	u8 val;

	/*
	 * MSB must be read before LSB because reading of MSB register
	 * latches the data.
	 */
	if (lp855x_read_byte(lp, TEMP_MSB, &val))
		return -1;
	temp = val << 3;

	if (lp855x_read_byte(lp, TEMP_LSB, &val))
		return -1;
	temp |= val >> 5;

	return (temp * 3495 - 2683100) / 10000;
}

static void temp_delayed_work(struct work_struct *work)
{
	struct delayed_work *delayed = to_delayed_work(work);
	struct lp855x *lp = container_of(delayed, struct lp855x, work);
	int temp;

	if (temp_trace_secs <= 0)
		return;

	temp = lp855x_get_temperature(lp);
	dev_info(lp->dev, "Backlight temperature: %d\n", temp);

	lp->running_work = 1;
	schedule_delayed_work(&lp->work, secs_to_jiffies(temp_trace_secs));
}
...
static int lp855x_probe(struct i2c_client *cl, const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&lp->work, temp_delayed_work);
	if (temp_trace_secs > 0)
		schedule_delayed_work(&lp->work, secs_to_jiffies(temp_trace_secs));
