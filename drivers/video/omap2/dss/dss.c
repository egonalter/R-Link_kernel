...
retry:
	...
	if (cpu_is_omap24xx()) {
		...
		for ( ; fck_div > 0; --fck_div) {
			...
			/* Search for the PCK frequency closest to the requested one. In case of 
			   equal frequencies, prefer even PCK divisors, that guarantee 50% duty cycle */
			if ((abs(cur_dispc.pck - req_pck) < abs(best_dispc.pck - req_pck)) || 
				((abs(cur_dispc.pck - req_pck) == abs(best_dispc.pck - req_pck)) &&
					cur_dispc.pck_div % 2 == 0)) {
				...
				if (cur_dispc.pck == req_pck && cur_dispc.pck_div % 2 == 0)
					goto found;
			}
		}
	} else if (cpu_is_omap34xx()){
