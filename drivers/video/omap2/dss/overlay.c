		...
		struct omap_overlay_manager *mgr)
{
	...
	/* TomTom: disable this delay, we have not seen this problem in either 
	 * strasbourg or santiago.
	 * This code gets called from several very limited paths:
	 *   1) from sysfs interface by using the manager sysfs file
	 * 		which is not used
	 *   2) from dss_recheck_connections, which is called only at startup
	 * 	always with lcd as the manager.
	 *
	 * Under no circumstance we change the current manager, so this sleep
	 * is superflous.
	 */
