...
static DEFINE_MUTEX(dpi_state_lock);
...
static int dpi_set_mode(struct omap_dss_device *dssdev)
{
	...
	if (use_dsi_for_hdmi || dssdev->phy.dpi.use_dsi_clk)
...
err0:
	if (cpu_is_omap44xx() && (use_dsi_for_hdmi || dssdev->phy.dpi.use_dsi_clk)) {
...
static int _dpi_display_resume(struct omap_dss_device *dssdev);
...
static void dpi_display_disable(struct omap_dss_device *dssdev)
{
	...
	mutex_lock(&dpi_state_lock);
	if (dssdev->state == OMAP_DSS_DISPLAY_DISABLED) {
		DSSDBG("dpi_display_disable: display DISABLED -> exit\n");
		goto end;
	}

	if (dssdev->state == OMAP_DSS_DISPLAY_SUSPENDED) {
		DSSDBG("dpi_display_disable: display SUSPENDED\n");
		_dpi_display_resume(dssdev);
	}
...
static int dpi_display_suspend(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->state != OMAP_DSS_DISPLAY_ACTIVE) {
		r = -EINVAL;
		DSSDBG("dpi_display_suspend: display NOT ACTIVE -> exit\n");
		goto end;
	}
...
/* only call this function after taking the dpi_state_lock mutex */
static int _dpi_display_resume(struct omap_dss_device *dssdev)
{
	int r = 0;
	int lcd_channel_ix = 1;
	...
	if (dssdev->state != OMAP_DSS_DISPLAY_SUSPENDED) {
		r = -EINVAL;
		DSSDBG("dpi_display_resume: display NOT SUSPENDED -> exit\n");
		goto err0;
	}
