...
int omap_vrfb_request_ctx(struct vrfb *vrfb)
{
	...
	static struct vrfb *prev_alloc;
	static int alloc_count = 0;
	
	/* For VRFB to work this hack was added. The first context is for the */
	/* standard FB. The next twelve are vor v4l2. Only 12 CTX are HW wise */
	/* possible. Assumption is that both (standard FB and 4vl2) are not   */
	/* used at the same time. So the first context (for FB) is being      */
	/* reused by v4l2 by following code. It seems to work fine. This      */
	/* allows us to use vrfb and rotate buffer using that method so we    */
	/* we can use the screen outside android as well.                     */
	if (alloc_count == 1) {
		*vrfb = *prev_alloc;
		alloc_count++;
		r = 0;
