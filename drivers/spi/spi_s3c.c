...
static irqreturn_t s3c_spi_interrupt(int irq, void *dev_id)
{
	...
	writel(0x1f, sspi->regs + S3C_SPI_PENDING_CLR);

	writel(0x20, sspi->regs);
...
static void handle_msg(struct s3cspi_bus *sspi, struct spi_message *msg)
{
	...
	if((sspi->cur_mode & SPI_READY) ^ (spi->mode & SPI_READY)) {
	sspi->cur_mode &= ~SPI_READY;
	sspi->cur_mode |= (spi->mode & SPI_READY);
...
static int s3c_spi_setup(struct spi_device *spi)
{
	...
	struct s3c_spi_mstr_info *smi = sspi->spi_mstinfo;
	//struct s3c_spi_pdata *spd = &sspi->spi_mstinfo->spd[spi->chip_select];  /have to delete 
...
#if 0
	if( (spi->mode & SPI_READY) && ( spd->cs_level == CS_FLOAT) )
