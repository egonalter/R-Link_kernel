...
static void mshci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->bus_width == MMC_BUS_WIDTH_8) { 
		...
		mshci_writel(host, (0x1<<0), MSHCI_CTYPE);
		mshci_writel(host, (0x1<<16), MSHCI_UHS_REG);
		printk("Change Host Controller to 4Bit DDR Mode\n");
	} else {
...
#ifdef CONFIG_MMC_MSHCI_S5P6450_FIFOTH_WORKAROUND
	fifo_val = 0x20080008;
#endif
...
int mshci_add_host(struct mshci_host *host)
{
	...
	mmc->ops = &mshci_ops;
	mmc->f_min = host->max_clk / 120; /*FIXME set init clock to 400KHz*/
