...
//	host->quirks |= SDHCI_QUIRK_NO_HISPD_BIT;
	...
	if (pdata->cfg_ext_cd) {
		...
		if(ret<0)
			printk("TONY ERROR <%s> error in request_irq = %d, line =%d \n",__func__, ret,pdata->ext_cd );
	}
...
#ifdef CONFIG_PM
struct sdhci_registers sdhci_reg;

static int sdhci_s3c_suspend(struct platform_device *dev, pm_message_t pm)
{
	struct sdhci_host *host = platform_get_drvdata(dev);
	struct s3c_sdhci_platdata *pdata = dev->dev.platform_data;
	sdhci_reg.blksize_reg = readw(host->ioaddr + 0x4);
	sdhci_reg.hostctl_reg = readb(host->ioaddr + 0x28);
	sdhci_reg.pwrcon_reg = readb(host->ioaddr + 0x29);
	sdhci_reg.timeoutcon_reg = readb(host->ioaddr + 0x2E);
	sdhci_reg.norintstsen_reg = readw(host->ioaddr + 0x34);
	sdhci_reg.errintstsen_reg = readw(host->ioaddr + 0x36);
	sdhci_reg.norintsigen_reg = readw(host->ioaddr + 0x38);
	sdhci_reg.errintsigen_reg = readw(host->ioaddr + 0x3A);
	sdhci_reg.control2_reg = readl(host->ioaddr + 0x80);
	sdhci_reg.control3_reg = readl(host->ioaddr + 0x84);
	sdhci_reg.control4_reg = readl(host->ioaddr + 0x8C);
...
static int sdhci_s3c_resume(struct platform_device *dev)
{
	...
	writew(sdhci_reg.blksize_reg, host->ioaddr + 0x4);
	writeb(sdhci_reg.hostctl_reg, host->ioaddr + 0x28);
	writeb(sdhci_reg.pwrcon_reg, host->ioaddr + 0x29);
	writeb(sdhci_reg.timeoutcon_reg, host->ioaddr + 0x2E);
	writew(sdhci_reg.norintstsen_reg,host->ioaddr + 0x34);
	writew(sdhci_reg.errintstsen_reg, host->ioaddr + 0x36);
	writew(sdhci_reg.norintsigen_reg, host->ioaddr + 0x38);
	writew(sdhci_reg.errintsigen_reg, host->ioaddr + 0x3A);
	writel(sdhci_reg.control2_reg, host->ioaddr + 0x80);
	writel(sdhci_reg.control3_reg, host->ioaddr + 0x84);
	writel(sdhci_reg.control4_reg, host->ioaddr + 0x8C);
