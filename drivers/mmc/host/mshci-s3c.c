 ...
 *
 * MSHCI (HSMMC) support for Samsung SoC
 *
...
static int __devinit mshci_s3c_probe(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, host);
        sc->clk_io = clk_get(dev, "hsmmc_44");
//	sc->clk_io = clk_get(dev, "mshc");
	...
	if(pdata->cd_type == S3C_MSHCI_CD_PERMANENT || pdata->cd_type == S3C_MSHCI_CD_NONE)
		host->quirks |= MSHCI_QUIRK_BROKEN_CARD_DETECTION; 

	host->mmc->boot_mode = boot_mode;
...
static int __devexit mshci_s3c_remove(struct platform_device *pdev)
{
	...
	struct mshci_s3c *sc = mshci_priv(host);

	mshci_remove_host(host, 0);
	release_resource(sc->ioarea);
	kfree(sc->ioarea);

	for (i=0; i<MAX_BUS_CLK; i++) {
		clk_disable(sc->clk_bus[i]);
		clk_put(sc->clk_bus[i]);
	}
...
#ifdef CONFIG_PM
struct mshci_registers mshci_reg;

static int mshci_s3c_suspend(struct platform_device *dev, pm_message_t pm)
{
	struct mshci_host *host = platform_get_drvdata(dev);

	printk("emmc suspend id =%d\n",dev->id);
	mshci_reg.ctrl_reg = readl(host->ioaddr + 0x00);
        mshci_reg.clkdiv_reg = readl(host->ioaddr + 0x08);
        mshci_reg.clksrc_reg = readl(host->ioaddr + 0x0c);
        mshci_reg.clkena_reg = readl(host->ioaddr + 0x10);
        mshci_reg.tmout_reg = readl(host->ioaddr + 0x14);
        mshci_reg.ctype_reg = readl(host->ioaddr + 0X18);
        mshci_reg.blksiz_reg = readw(host->ioaddr + 0x1c);
        mshci_reg.intmask_reg = readl(host->ioaddr + 0x24);
        mshci_reg.mintsts_reg = readl(host->ioaddr + 0x40);
...
static int mshci_s3c_resume(struct platform_device *dev)
{
	...
	printk("emmc resume id=%d\n", dev->id);

	writel(mshci_reg.ctrl_reg, host->ioaddr + 0x00);
        writel(mshci_reg.clkdiv_reg, host->ioaddr + 0x08); 
        writel( mshci_reg.clksrc_reg, host->ioaddr + 0x0c);
        writel(mshci_reg.clkena_reg, host->ioaddr + 0x10);
        writel(mshci_reg.tmout_reg, host->ioaddr + 0x14);
        writel(mshci_reg.ctype_reg, host->ioaddr + 0X18);
        writew(mshci_reg.blksiz_reg, host->ioaddr + 0x1c);
        writel(mshci_reg.intmask_reg, host->ioaddr + 0x24);
        writel(mshci_reg.mintsts_reg, host->ioaddr + 0x40);
	mshci_resume_host(host);
	//__raw_writel(0x10,host->ioaddr);
	return 0;
}
...
module_param(boot_mode, uint, 0644);
MODULE_PARM_DESC(boot_mode, "boot mode: 0=[normal mode], 1=boot mode");
