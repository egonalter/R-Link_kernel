...
static void mxt_input_report(struct mxt_data *data, int single_id)
{
	...
	if(finger_num > 2){
		printk("ATMEL-TS: more than 2 fingers on screen!\n");
		input_report_abs(input, ABS_FINGERS, 0);
		input_report_abs(input, ABS_PRESSURE, 0);
		goto sync;
	}
	/* Normally with multitouch all fingers could be reported in the loop and the finger_number 
	 * indicates which ones have relevant data, however in our case we don't
	 * use the ABS_MT_POSITION_X code, we have added ABS_FINGER2_X so we need to add
	 * the values here.  	
	 */
	input_report_abs(input, ABS_X, finger[0].x);
	input_report_abs(input, ABS_Y, finger[0].y);
	input_report_abs(input, ABS_FINGER2_X, finger[1].x);
	input_report_abs(input, ABS_FINGER2_Y, finger[1].y);
	input_report_abs(input, ABS_FINGERS, finger_num);
	input_report_key(input, BTN_TOUCH, finger_num > 0);
	
	if(finger_num)
		input_report_abs(input, ABS_PRESSURE, finger[0].area);
	else 
		input_report_abs(input, ABS_PRESSURE, 0);
...
//	finger[id].x = x;
//	finger[id].y = y;
	/* Convert into old RTP coordinates */
	finger[id].x = x;
	finger[id].y = 4095 - y;
	finger[id].area = area;
...
static int mxt_make_highchg(struct mxt_data *data)
{
        ...
        do {
                ret = mxt_read_reg(data->client, data->T5_address,
                        data->T5_msg_size, msg);

                reportid = msg[0];

                mxt_proc_message(data, msg);
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	int ret;
	u8 command_register;
	char mxt_cfg_name[NAME_MAX];

	sprintf(mxt_cfg_name, "%s"MXT_CFG_NAME_EXT, data->pdata->screen_name);
