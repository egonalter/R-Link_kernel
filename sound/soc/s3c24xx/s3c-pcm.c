...
static struct s3c_dma_params s3c_pcm_stereo_out[MAX_PCM];
static struct s3c_dma_params s3c_pcm_stereo_in[MAX_PCM];
static struct s3c_pcm_info s3c_pcm[MAX_PCM];
                                ...
                                struct snd_soc_dai *dai)
{
	...
	if (!tx_clk_enabled && !rx_clk_enabled) {
		if (reg_saved_ok == true) {
			...
			pr_debug("PCM Audio Clock enabled and Registers restored...\n");
		}
	}
...
static void s3c_pcm_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (!tx_clk_enabled && !rx_clk_enabled) {
		...
		pr_debug("PCM Audio Clock disabled and Registers stored...\n");
	}
	return;
}
#endif
...
#define S3C_PCM_FORMATS  (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_U8)
...
static __devinit int s3c_pcm_dev_probe(struct platform_device *pdev)
{
	...
	struct clk *fout_epll, *mout_epll, *mout_audio,*iis_clk;
	int ret;
	...
	/* Initialize the DAI structure for this controller */
	s3c_pcm_dai[pdev->id].name = "samsung-pcm";
	s3c_pcm_dai[pdev->id].id = pdev->id;
	s3c_pcm_dai[pdev->id].symmetric_rates = 1;
	s3c_pcm_dai[pdev->id].ops = &s3c_pcm_dai_ops;
	s3c_pcm_dai[pdev->id].playback.channels_min = 1;
	s3c_pcm_dai[pdev->id].playback.channels_max = 2;
	s3c_pcm_dai[pdev->id].playback.rates = S3C_PCM_RATES;
	s3c_pcm_dai[pdev->id].playback.formats = S3C_PCM_FORMATS;
	s3c_pcm_dai[pdev->id].capture.channels_min = 1;
	...
	clk_set_parent(pcm->cclk, mout_epll);
	clk_enable(pcm->cclk);
	...
	if(pdev->id == 0)
		iis_clk = clk_get(&pdev->dev, "i2s_v40");
	else
		iis_clk = clk_get(&pdev->dev, "iis");
#else
	iis_clk = clk_get(&pdev->dev, "iis");
#endif
	if (IS_ERR(iis_clk)) {
		dev_err(&pdev->dev, "failed to get iis_clk\n");
		ret = -ENOENT;
		goto err4;
	}
	clk_enable(iis_clk);

	pcm->pclk = clk_get(&pdev->dev, "pclk_pcm");
	if (IS_ERR(pcm->pclk)) {
	...
	if (ret != 0) {
		dev_err(&pdev->dev, "snd_soc_register_dai failed to get pcm_clock\n");
		goto err5;
	}

	s3c_pcm_stereo_in[pdev->id].client = &s3c_pcm_dma_client_in;
	s3c_pcm_stereo_out[pdev->id].client = &s3c_pcm_dma_client_out;

	/* NOTE: this value doesn't work properly for all codecs (see machine specific codecs) */
	s3c_pcm_stereo_in[pdev->id].dma_size = 4;
	s3c_pcm_stereo_out[pdev->id].dma_size = 4;
...
{
	struct snd_soc_dai *dai = &s3c_pcm_dai[pdev->id];
