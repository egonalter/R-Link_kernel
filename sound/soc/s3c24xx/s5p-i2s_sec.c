...
{
	ucontrol->value.integer.value[0] = to_info(&i2s_sec_fifo_dai)->use_idma;
...
static int set_dma_mode(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	if (to_info(&i2s_sec_fifo_dai)->use_idma == ucontrol->value.integer.value[0]
		|| i2s_sec_fifo_dai.active)
		return 0;

	to_info(&i2s_sec_fifo_dai)->use_idma = ucontrol->value.integer.value[0];
