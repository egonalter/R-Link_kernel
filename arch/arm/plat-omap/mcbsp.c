...
void omap_mcbsp_config(unsigned int id, const struct omap_mcbsp_reg_cfg *config)
{
	...
	spcr2_reg = config->spcr2;
	if (mcbsp->pdata->clk_always_on) {
		if (config->srgr1 == OMAP_MCBSP_READ(mcbsp->io_base, SRGR1) &&
		    config->srgr2 == OMAP_MCBSP_READ(mcbsp->io_base, SRGR2) &&
		    config->pcr0 == OMAP_MCBSP_READ(mcbsp->io_base, PCR0))
			spcr2_reg |= (FRST | GRST);
		else
			spcr2_reg &= ~(FRST | GRST);
	}
	...
	OMAP_MCBSP_WRITE(io_base, SPCR2, spcr2_reg);
...
int omap_mcbsp_request(unsigned int id)
{
	...
	OMAP_MCBSP_WRITE(mcbsp->io_base, SPCR2, (mcbsp->pdata->clk_always_on ? (FRST | GRST) : 0));
...
static int __devinit omap_mcbsp_probe(struct platform_device *pdev)
{
	...
	/* Enable the fck from the beginning */
	if (mcbsp->pdata->clk_always_on)
		clk_enable(mcbsp->fclk);
