...
struct omap_sr {
	...
	u32		opp4_nvalue, opp5_nvalue, opp6_nvalue;
	...
	if (sr->srid == SR1) {
		if (cpu_is_omap3630()) {
			...
			if (sr->opp5_nvalue != 0x0) {
				...
				pr_info("SR: Nvalues not fused for "
							"1.2G, disabled\n");
				}
			...
			if (sr->opp3_nvalue != 0) {
				pr_info("SR2:Fused Nvalues for VDD1OPP3 %d\n",
							sr->opp3_nvalue);
			} else {
				pr_info("SR: Nvalues not fused for "
							"800Mhz, use test values\n");
				/* use test nvalues */
			...
			if (sr->opp2_nvalue != 0) {
				pr_info("SR2:Fused Nvalues for VDD1OPP2 %d\n",
							sr->opp2_nvalue);
			} else {
				/* use test nvalues */
				sr_set_testing_nvalues(sr);
			...
			if (sr->opp1_nvalue != 0) {
				pr_info("SR2:Fused Nvalues for VDD1OPP1 %d\n",
							sr->opp1_nvalue);
			} else {
				/* use test nvalues */
				sr_set_testing_nvalues(sr);
