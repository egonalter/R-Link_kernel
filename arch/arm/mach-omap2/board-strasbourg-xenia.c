...
static void __init strasbourg_mmc_init(void)
{
	u32 prog_io = omap_ctrl_readl(OMAP343X_CONTROL_PROG_IO0);

	/* Program (bit 1)=1 to select 10k-50k Ohms strength on SDMMC IO cells */
...
#endif
    printk(KERN_INFO "--- strasbourg_init done for Xenia board file ---\n");
}
...
MACHINE_START(STRASBOURG_A2, "Xenia")
	.phys_io	= 0x48000000,
