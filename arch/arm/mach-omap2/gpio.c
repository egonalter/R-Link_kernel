...
#define OMAP34XX_PAD_IN_PU_GPIO (OMAP_MUX_MODE4 | OMAP_PIN_INPUT_PULLUP)
#define OMAP34XX_PAD_IN_PD_GPIO (OMAP_MUX_MODE4 | OMAP_PIN_INPUT_PULLDOWN)
...
void omap_gpio_save_context(void)
{
	...
	while (pad->gpio >= 0) {
		...
		/* check if gpio is configured as output => need hack for erratum i468 */
		if (!(tmp_oe[i] & pin)) {
			/* save current padconf setting */
			pad->save = omap_ctrl_readw(offset);
			out = gpio_context[i].dataout;

			/* Check if output OFF mode has been configured */
			if ( (pad->save & OMAP_OFF_EN) &&
			    !(pad->save & OMAP_OFFOUT_EN)) {
				/* Use configured OFF mode setting */
				if (pad->save & OMAP_OFFOUT_VAL)
					/* High: PU + input */
...
int __init omap_gpio_early_init(void)
{
	int i = 0;
