...
static int s3c_pwm_probe(struct platform_device *pdev)
{
	...
	if (id == 0) {
		printk("\n %s gpio_is_valid(S5P6450_GPF(14)) === %d",__func__,gpio_is_valid(S5P6450_GPF(14)));
		if(gpio_is_valid(S5P6450_GPF(14))) {
			ret = gpio_request(S5P6450_GPF(14), "GPF");

			if (ret) {
				printk(KERN_ERR "failed to request GPF for PWM-OUT 0\n");
			}
			s3c_gpio_cfgpin(S5P6450_GPF(14),S5P64XX_GPF14_PWM_TOUT0);
		}
	} else if(id == 1) {
		if(gpio_is_valid(S5P6450_GPF(15))) {
			ret = gpio_request(S5P6450_GPF(15), "GPF");

			if (ret) {
				printk(KERN_ERR "failed to request GPF for PWM-OUT 1\n");
			}
			s3c_gpio_cfgpin(S5P6450_GPF(15),S5P64XX_GPF15_PWM_TOUT1);			 
	...
	unsigned int gpfcon  = __raw_readl(S5P64XX_GPFCON);
	/* printk("\n %s::gpfcon =0x%x",__func__,gpfcon); 
	printk("\n %s::Direct address of gpfcon =0x%x",__func__,S5P64XX_GPFCON);*/
	//Temporary fix for backlight ::TODO 
	__raw_writel(0xa0000000,S5P64XX_GPFCON);
