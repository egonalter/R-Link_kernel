...
static int smi_getclcks(struct s3c_spi_mstr_info *smi)
{
	...
	if(cspi == NULL){
		cspi = clk_get(&smi->pdev->dev, "pclk_spi");
		if(IS_ERR(cspi)){
			printk("Unable to get clock spi!\n");
			return -EBUSY;
		}
	}
...
static struct resource s3c_spi0_resource[] = {
	[0] = {
		.start = S5P6450_PA_SPI0,
		.end   = S5P6450_PA_SPI0 + S5C_SZ_SPI0 - 1,
...
static struct resource s3c_spi1_resource[] = {
	[0] = {
		.start = S5P6450_PA_SPI1,
		.end   = S5P6450_PA_SPI1 + S5C_SZ_SPI1 - 1,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
		.start = IRQ_SPI1,
