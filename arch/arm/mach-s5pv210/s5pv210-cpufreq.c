...
/* frequency */
static struct cpufreq_frequency_table s5pc110_freq_table_normal[] = {
...
static struct cpufreq_frequency_table s5pc110_freq_table_restrict[] = {
	{L0, 1000*1000},	/* OVerclocked */
	{L1, 800*1000},
	{L2, 400*1000},
	{L3, 200*1000},
...
static struct cpufreq_frequency_table *s5pc110_freq_table[] = {
	s5pc110_freq_table_normal,
	s5pc110_freq_table_restrict,
};
...
static int cpufreq_change_count = 0;
void s5pc110_set_cpufreq_level(int flag)
{
	spin_lock(&cpufreq_lock);
	if (flag == NORMAL_STATE) {
		       ...
		       unsigned int relation)
{
	...
	if (cpufreq_frequency_table_target(policy, s5pc110_freq_table[S5PC11X_FREQ_TAB],
...
#ifdef CONFIG_PM
static int s5pc110_cpufreq_suspend(struct cpufreq_policy *policy,
			pm_message_t pmsg)
{
	int ret = 0;
