...
struct platform_device valdez_sec_device_battery = {
	.name	= "sec-fake-battery",
	.id				= -1,
};

static struct platform_device valdez_suicide_alarm = {
	.name 	= "suicide_alarm",
	.id	= -1
};

struct map_desc valdez_iodesc[] = {};
...
static struct s5m8751_power_pdata smdk6450_power_pdata = {
        ...
        .gpio_hadp_lusb = S5P6450_GPP(3),
};

static struct s5m8751_audio_pdata smdk6450_audio_pdata = {
	.dac_vol = 0x24, /* -6 dB, boot with limited output !!, current consumption */
	.hp_vol = 0x14,
};
...
static struct platform_pwm_backlight_data smdk_backlight_data = {
	.pwm_id		= 1,
	.max_brightness	= 255,
	.dft_brightness	= 128,	/* boot with 50% brightness, to limit current consumption !! */
	.pwm_period_ns	= 78770,
};
...
{
	s5p_init_io(valdez_iodesc, ARRAY_SIZE(valdez_iodesc), S5P_SYS_ID);
	s3c_device_nand.name = "s5p6450-nand";
	//s5p64xx_init_io(valdez_iodesc, ARRAY_SIZE(valdez_iodesc));
...
// Pin tables
static struct vgpio_pin valdez_vgpio0_pins[] = {
	TT_DEF_NCPIN0(WIFI_RST),
	TT_DEF_PIN0	(TILT_PWR,	S5P6450_GPH(9)),
	TT_DEF_PIN0	(TILT_OUT,	S5P6450_GPH(7)),
	TT_DEF_INVPIN0(TP_RESET, S5P6450_GPB(2)),
	TT_DEF_PIN0(TP_IRQ, S5P6450_GPN(7)),
	TT_DEF_INVPIN0(ON_OFF, S5P6450_GPN(4)),
	TT_DEF_PIN0(RFS_BOOT_CLK, S5P6450_GPG(13)),
	TT_DEF_PIN0(RFS_BOOT_Q, S5P6450_GPH(6)),
	TT_DEF_NCPIN0(EEPROM_WP),
	TT_DEF_INVPIN0(MIC_RESET, S5P6450_GPF(15)),
	TT_DEF_PIN0(AMP_PWR_EN, S5P6450_GPP(4)),
	TT_DEF_NCPIN0(MIC_AMP_EN),
	...
	TT_DEF_INVPIN0(LCM_PWR_EN, S5P6450_GPP(6)),
	...
	TT_DEF_NCPIN0(SD_PWR_EN),
	TT_DEF_NCPIN0(MUTE),
	TT_DEF_NCPIN0(SD_CD),
	TT_DEF_NCPIN0(ACCESSORY_PWR_EN),
	TT_DEF_PIN0(GSM_SYS_EN, S5P6450_GPR(11)),
	TT_DEF_PIN0(GSM_SYS_RST, S5P6450_GPR(12)),

};

// Platform data & device
static struct vgpio_platform_data valdez_vgpio_pdata[] = {
	[0] = {
        	.gpio_base      = TT_VGPIO0_BASE,
        	.gpio_number    = ARRAY_SIZE(valdez_vgpio0_pins),
        	.pins           = valdez_vgpio0_pins,
	},
};

static struct platform_device valdez_vgpio[] = {
	[0] = {
        	.name           = "vgpio",
        	.id             = 0,
        	.dev            = {
                .platform_data  = &valdez_vgpio_pdata[0],
        	},
	},
};

static void __init valdez_lcd_init(void)
{
...
/*--------Below are the sleep mode configurations for eMMC singal (GPQ6-9, GPG9-12)--*/

	/* output low so we dont leak into vdd lines */
	tmp = __raw_readl(S5P64XX_GPGCONSLP);
	tmp &= ~0x000FF000; 
        __raw_writel(tmp,S5P64XX_GPGCONSLP);
	/* output low so we dont leak into vdd lines */
	tmp = __raw_readl(S5P64XX_GPQCONSLP);
	tmp &= ~0x03FC0000;
        __raw_writel(tmp,S5P64XX_GPQCONSLP);

/* configure sleep state for lcm_power; it needs to be output high to disable lcm power */
	tmp = __raw_readl(S5P64XX_GPPCONSLP);
	tmp &= ~0x00003000; 
	tmp |=  0x00001000; /*set output high*/
        __raw_writel(tmp,S5P64XX_GPPCONSLP);
}
...
static void __init valdez_machine_init(void)
{
	platform_device_register(&valdez_vgpio);
...
MACHINE_START(VALDEZ, "Valdez")
	...
	.init_machine	= valdez_machine_init,
	.timer		= &s5p_systimer,
MACHINE_END
