...
unsigned int ubootversion;
EXPORT_SYMBOL(ubootversion);
...
{
	fdt_buffer_info.address = (unsigned long *) tag->u.factorydata.address;
	fdt_buffer_info.size = (size_t) tag->u.factorydata.size;
...
static int __init parse_tag_ubootversion(const struct tag *tag)
{
	ubootversion = tag->u.ubootversion.ver;
	return 0;
}

__tagtable(ATAG_UBOOTVERSION, parse_tag_ubootversion);
...
static int c_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "Uboot\t\t: %d\n", ubootversion);
#if defined(CONFIG_TOMTOM_VERSION)
	seq_printf(m, "Kernel\t\t: %s\n", CONFIG_TOMTOM_VERSION);
#endif
